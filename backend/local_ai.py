...
def score_call(transcript: list) -> dict:
    ...
    sys_prompt = (
        ...
        "STRICT OUTPUT RULE: Return ONLY a valid JSON object, with no markdown, no code fences, no explanation, and no extra text. "
        "The JSON object must look exactly like: {\"summary\": \"...\", \"lead_score\": 5, \"final_intent\": \"Neutral\"}. "
        "If you cannot comply, output {\"summary\": \"Unable to score\", \"lead_score\": 0, \"final_intent\": \"Neutral\"}."
    )
    tx_str = "\n".join([
    ...
    try:
        ...
        return {
            "summary": "Scoring error: Ollama is offline or unreachable. Start Ollama with 'ollama serve'.",
            "lead_score": 0,
            "final_intent": "Neutral"
        }
    except requests.exceptions.Timeout:
        return {"summary": "Scoring error: Ollama scoring request timed out", "lead_score": 0, "final_intent": "Neutral"}
    except Exception as e:
